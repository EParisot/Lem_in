...
/*   Updated: 2018/05/09 18:57:33 by eparisot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	read2bis(t_ant_hill *ant_hill, char *line, t_list *tmp_lst)
{
	...
	{
		get_next_line(0, tmp);
		if (!p_c(ant_hill, tmp) || !(read2_bis(ant_hill, *tmp, tmp_lst, NULL)))
			return (0);
	...
	{
		get_next_line(0, tmp);
		if (!p_c(ant_hill, tmp) || !(read2_ter(ant_hill, *tmp, tmp_lst, NULL)))
			return (0);
		save_instru(ant_hill, *tmp);
		free(*tmp);
	}
...
int			read2ter(t_ant_hill *ant_hill, char *line, t_list *tmp_lst)
{
	...
	{
		get_next_line(0, tmp);
		if (!p_c(ant_hill, tmp) || !(read3(ant_hill, *tmp, tmp_lst, tb)))
			return (0);
		save_instru(ant_hill, *tmp);
		free(*tmp);
	}
